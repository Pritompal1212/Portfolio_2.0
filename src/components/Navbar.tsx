...
//new one with some error handle
"use client";
import React, { useState, useEffect, useRef } from "react";
import { motion, AnimatePresence } from "framer-motion";
...
const Navbar: React.FC<{ className?: string }> = ({ className }) => {
  ...
  return (
    <div className={`fixed top-1 md:top-10 inset-x-0 md:w-[50%] mx-auto z-50 ${className}`}>
      <nav
        className={`flex justify-between items-center px-6 py-4 rounded-full shadow-md transition-all duration-300 backdrop-blur-md border border-white/20 dark:border-white/10 bg-white/10 dark:bg-black/20 ${
          hasScrolled ? "shadow-xl" : "shadow-md"
        }`}
      >
        ...
        <div className="md:hidden flex items-center space-x-4">
          ...
          >
            ...
            >
              <motion.path
                d={isMenuOpen ? "M6 18L18 6" : "M3 7h18"}
                initial={false}
                animate={{ d: isMenuOpen ? "M6 18L18 6" : "M3 7h18" }}
              />
              <motion.path
                d="M3 12h18"
                initial={false}
              ...
              />
              <motion.path
                d={isMenuOpen ? "M6 6l12 12" : "M3 17h18"}
                initial={false}
                animate={{ d: isMenuOpen ? "M6 6l12 12" : "M3 17h18" }}
              />
            </svg>
          </button>
        </div>
